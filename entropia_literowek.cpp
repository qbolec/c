...
map<string,string> slowa;
string letsort(string s){
  string ret=s;
  sort(ret.begin()+1,ret.end()-1);
  return ret;
}
void literowki(unsigned char * dupa){
  while(*dupa){
    while(*dupa && !('a'<=*dupa && *dupa<='z' || 'A'<=*dupa && *dupa<='Z'))dupa++;
    unsigned char * e=dupa;
    while(*e && ('a'<=*e && *e<='z' || 'A'<=*e && *e<='Z'))e++;
    if(dupa+1<e-1){      
      string s((char*)dupa,e-dupa);
      string h=letsort(s);
      if(slowa.find(h)!=slowa.end()){
        string z=slowa[h];
        if(z!=s){
          cout << "Zamieniam " << s << " na " << z<< endl;
          for(int i=z.length();i--;)
            dupa[i]=z[i];          
        }
      }
    }
...
string toang(string pol){
  string res=pol;
  for(int i=pol.length();i--;){
    if((unsigned char)pol[i]==190)res[i]='z';//¿
    else if((unsigned char)pol[i]==149)res[i]='z';//Ÿ
    else if((unsigned char)pol[i]==165)res[i]='a';
    else if((unsigned char)pol[i]==169)res[i]='e';
    else if((unsigned char)pol[i]==134)res[i]='c';
    else if((unsigned char)pol[i]==136)res[i]='l';
    else if((unsigned char)pol[i]==152)res[i]='s';
    else if((unsigned char)pol[i]==228)res[i]='n';
    else if((unsigned char)pol[i]==162)res[i]='o';
  }
  return res;
}
